/**
 * CLI for interacting with silent-bell contract
 */
...
import { witnesses, emptyPrivateState } from './witnesses';
...
const SilentBell = await import(pathToFileURL(contractPath).href);
...
async function main() {
  ...
  try {
    ...
    const deployed: any = await findDeployedContract(providers, {
      ...
      initialPrivateState: emptyPrivateState(randomBytes(32)),
    });
    ...
    while (running) {
      console.log('─── Menu ───────────────────────────────────────────────────────');
      console.log('  1. (write circuits via UI / callTx — coming next)');
      console.log('  2. Read ledger counts');
      ...
      switch (choice.trim()) {
        ...
        case '2': {
          ...
          try {
            ...
            if (contractState) {
              const ledgerState = SilentBell.ledger(contractState.data);
              console.log(`\n  silentCount: ${ledgerState.silentCount}`);
              console.log(`  namedCount: ${ledgerState.namedCount}`);
              console.log(`  epoch: ${ledgerState.epoch}\n`);
            } else {
